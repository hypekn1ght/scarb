...
impl<'c> Source for RegistrySource<'c> {
    ...
    async fn query(&self, dependency: &ManifestDependency) -> Result<Vec<Summary>> {
        let before_network = {
            ...
            let network_allowed = self.config.network_allowed();
            Box::new(move || {
                ensure!(
                    network_allowed,
                    "cannot query `{source_id}` in offline mode"
                );
                Ok(())
            })
        };
        ...
        let records = self
            .client
            .get_records_with_cache(dependency, before_network)
            .await
            .with_context(|| {
                format!(
    ...
    async fn download(&self, id: PackageId) -> Result<Package> {
        let before_network = {
            ...
            Box::new(move || {
                ...
                ui.print(Status::new("Downloading", &id.to_string()));
        ...
        let archive = self
            .client
            .download_with_cache(id, before_network)
            .await
...
impl<'c> RegistrySource<'c> {
    ...
    async fn load_package(&self, id: PackageId, archive: FileLockGuard) -> Result<Package> {
        ...
        let archive_path = archive.path().to_path_buf().into_std_path_buf();
        let path = self.package_sources.extract(id, archive_path).await?;

        // We don't need the archive anymore, release locks.
        drop(archive);
